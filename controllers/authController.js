...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(new AppError("The user doesn't exist anymore", 404));
  }
...
exports.isLoggedIn = catchAsync(async (req, res, next) => {
  if (req.cookies.jwt) {
    ...
    // 3) Check if user chaged password after the token was issued
    if (currentUser.changesPasswordAfter(decoded.iat)) {
      return next();
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not  exprired, and there is user, set this new password
  if (!user) {
